...
当前交付目标是“比赛可演示、可继续试点”的完整 MVP，不把尚未采购的短信、真人声线克隆或推送服务伪装成已经上线。
...
- DeepSeek Chat Completions 真实联网对话；未填写密钥时使用安全演示模型。
- AI 身份常驻提示，诈骗、转账、医疗、自伤、迷路、疑似虐待与情感依赖保护。
- 家庭资料分块检索、已确认个人记忆、证据来源展示和提示词注入防护。
- 子女表达习惯档案；必须由老人单独授权，且 AI 始终不能冒充真人。
...
- 若要真人仿真声线，另购支持本人授权、删除和水印的合规 TTS/声线供应商。

服务器可使用 `docker-compose.yml` 部署 PostgreSQL、Redis 和 API；由于本机不能再虚拟化，该配置只用于云服务器，不是本机运行前提。

## 明确边界

- “关怀摘要”不是医学健康诊断，只汇总老人明确授权的主题、需求和互动信号。
- 当前对话语音输入/朗读使用手机或浏览器系统能力；DeepSeek 负责语言理解与生成。
- 家人语音留言是真实音频上传与播放；一比一真人声线克隆需要外部供应商 API 和单独合规审核。
- 当前提醒在 App 内展示并记录完成状态；锁屏推送、短信或电话提醒需要上线时接入 APNs/FCM/厂商推送。
- SQLite 适合单机演示；多人试点应切换 PostgreSQL，并使用正式迁移、密钥管理和备份策略。

完整设计依据见 [项目开发文档](docs/PROJECT_DEVELOPMENT_DOCUMENT.md) 和 [MVP API](docs/api/MVP_API.md)。
